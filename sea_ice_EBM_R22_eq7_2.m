...
%
%--------------------------------------------------------------------------
function [t_c, T_c] = sea_ice_EBM_R22_eq7_2(varargin) % STILL BUGGY
%%Model parameters --------------------------------------------------------
...
% maybe cooler here to write insolation as ck*sin(t-pk) rather than ak*cos(t)+bk*sin(t)
ck=sqrt(ak.^2+bk.^2).*sign(ak); pk=atan(ak./bk);
% XX this only appears to work up to truncations of about 40 - not sure why.
...
for k=1:trunc % sum up analytical solutions
    ...
    % can write this also as C*sin(t-P)
    C=a*ck(k) / sqrt( B^2 + (2*pi*k*cw)^2 );
    P=atan( ( -2*pi*k*cw + B*tan(pk(k)) ) / ( B + 2*pi*k*cw*tan(pk(k)) ) );
    T = T + C*sin(2*pi*k*t + P);
end
